...
| 請假申請通知（`leave_requested`，2026-08-30）：學生／家長送出請假時推播給**授課老師與所有管理員**（主系統以 Map 依 user_id 去重，管理員兼授課只收一則）。原本送出後沒有任何人會收到通知，老師可能到上課才發現人沒來。本系統新增 `NOTIFY_TYPES.LEAVE_REQUESTED`、notify route case（走 `generalNotifyMessage`，抬頭「請假申請通知」）、後台開關預設值與兩處標籤。**注意**：此類型的 `student_name` 帶的是**收件人**（老師／管理員）姓名而非請假學生，請假學生姓名在訊息本文 | ✅ |
